(function () {
    function getQueryVariable(variable) {
        ...
        for (var i = 0; i < vars.length; i++) {
            ...
            if (pair[0] === variable) {
                return decodeURIComponent(pair[1].replace(/\+/g, ' ')).trim();
            }
        }
    }
    ...
    function displaySearchResults(results, query) {
        ...
        if (results.length) {
            ...
            searchProcessEl.innerText = "Prikaži";
        } else {
            searchResultsEl.innerHTML = '<p>Nema rezultata za "' + query + '"</p>';
            searchProcessEl.innerText = "Pretraži";
        }
    }

    var query = getQueryVariable("q") || "";
    var searchQueryContainerEl = document.getElementById("search-query-container");
